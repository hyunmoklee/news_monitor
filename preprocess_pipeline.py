...
"""
Doosan Enerbility News Preprocessing Pipeline (Stages 1 to 9)
...
def load_config(config_path: str = "filter_config.yaml") -> dict:
...
async def run_pipeline():
    target_keyword = "두산에너빌리티"
    print("==================================================================")
    print(f"🚀 Starting {target_keyword} Preprocessing Pipeline (Audit Trail, Global & Focus)")
    print("==================================================================")
    
    config = load_config("filter_config.yaml")
    await db.init_db()
    
    # 1. Fetch RAWDATA from articles table for Doosan Enerbility
    async with aiosqlite.connect(DB_PATH) as conn:
        conn.row_factory = aiosqlite.Row
        async with conn.execute(
            "SELECT * FROM articles WHERE keyword = ? OR keyword IS NULL ORDER BY published_at DESC",
            (target_keyword,)
        ) as cursor:
    ...
    if total_raw == 0:
        print("[!] No raw articles found for Doosan Enerbility.")
        return
    ...
    if cleaned_articles:
        print("\n[*] Step 3: Extracting Kiwi Nouns/Numbers & Reducing Candidate Pairs (Jaccard >= 0.04)...")
        candidate_pairs = generate_candidate_pairs(cleaned_articles, min_jaccard=config.get("embedding_dedup", {}).get("min_jaccard_candidate", 0.04))
    ...
    print(f"\n[*] Step 9: Saving {len(all_final)} articles with Full Audit Trail to [processed_articles] table...")
    ...
    print(f"\n==================================================================")
    print(f"🎉 {target_keyword} Preprocessing Pipeline Complete Summary:")
