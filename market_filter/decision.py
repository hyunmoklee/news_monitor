...
async def call_gemini_fallback(
    ...
    prompt = (
        ...
        f"   - 【산업 생태계 및 정책 영향】 [{t_name}]의 사업/수주/공급망에 직접적인 영향을 미치는 국가 간 원전/반도체 정책, 정부 수주 협상, 지재권 갈등, 밸류체인 심층 분석 기사인 경우 기업 기사로 분류하세요.\n"
        ...
        f"2. [시황/노이즈 기사 (is_market_news: true)]:\n"
        f"   - 코스피/코스닥 지수 시황, 외인/기관/개미 매매 동향, 복수 종목을 단순 나열한 기계적 시세 기사.\n"
        f"   - [{t_name}]의 고유 비즈니스 내용 없이 단순 주가/호가 등락만 1~2줄 나열한 시세봇 기사.\n"
        f"   - 【주체성 결여】 상위 그룹사/지주사/총수 회장 개인의 지배구조/사업재편/인물탐구 기사에서 [{t_name}]이 단순 계열사 목록 중 하나로 1~2회 스쳐 지나간 경우.\n\n"
