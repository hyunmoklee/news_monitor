        ...
        function renderProcessedTable() {{
            ...
            list.forEach((item, idx) => {{
                ...
                if (!item.is_market_news && !item.is_exact_dup) {{
                    tagsHtml += `<div style="padding:0.2rem 0.5rem; border-radius:4px; font-size:0.72rem; font-weight:700; background:rgba(16,185,129,0.2); color:#10b981; border:1px solid rgba(16,185,129,0.4); margin-bottom:0.3rem;"><i class="fa-solid fa-building-circle-check"></i> 🏢 기업 핵심</div>`;
                }} else {{
                    tagsHtml += `<div style="padding:0.2rem 0.5rem; border-radius:4px; font-size:0.72rem; font-weight:700; background:rgba(249,115,22,0.15); color:#f97316; border:1px solid rgba(249,115,22,0.4); margin-bottom:0.3rem;"><i class="fa-solid fa-chart-line"></i> 📈 단순 시황</div>`;
                }}

                if (item.focus_type === 'EXCLUSIVE_SOLO') {{
                    tagsHtml += '<div style="padding:0.15rem 0.4rem; border-radius:4px; font-size:0.7rem; font-weight:700; background:var(--solo-bg); color:var(--solo-color); margin-top:0.2rem;">🎯 SOLO 단독</div>';
                }}
                if (item.is_global_relevant) {{
                    tagsHtml += '<div style="padding:0.15rem 0.4rem; border-radius:4px; font-size:0.7rem; font-weight:700; background:var(--global-bg); color:#60a5fa; margin-top:0.2rem;">🌐 GLOBAL 투자</div>';
                }}
                ...
                tr.innerHTML = `
                    ...
                    <td style="text-align: center;">
                        ${{tagsHtml}}
                        <span class="cat-badge ${{catClass}}" style="font-size:0.72rem;"><i class="fa-solid ${{catIcon}}"></i> ${{cat}}</span>
                    </td>
                    <td>
